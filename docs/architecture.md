...
- **Storage:** persisted on the `users` table via `selected_month` /
  ...
  when unset; `User::setSelectedPeriod($month, $year)` persists a change.
...
- **Consumers:** screen components `use` the
  ...
  refresh in-place when the period changes without a navigation.
...
**before** trait mount hooks, so resolve the period via `currentSelectedPeriod()`
(reads the user) rather than the not-yet-set `periodMonth`/`periodYear` when
initialising other state inside `mount()`.
